...
void CpuSpinTask(void *arg){
	...
	while (1){
		...
		if(spincnt < BUF_SIZE){
			...
			BufSpinJitter[spincnt] = MathAbsValI((int)SpinTaskPeriod - (int)ECATCTRL_TASK_PERIOD);
			BufSpinExec[spincnt] = (int)execend - (int)start;
			end = start;
			spincnt++;
		}
		//rt_printf("%d.%06d\n",SpinTaskPeriod/1000000, SpinTaskPeriod%1000000);
	}
}
#endif
...
int main(int argc, char **argv){
	...
	FilePrintEval((char *)"stresstask.csv",BufSpinPeriodTime,BufSpinExec,BufSpinJitter,BufEcatCollect, BufEcatProcess, BufEcatTranslate,BufSpinTime,spincnt);
...
int XenoInit(void){
	...
	if(rt_task_create(&TskEcatCtrl,"EtherCAT Control", 0, ECATCTRL_TASK_PRIORITY,ECATCTRL_TASK_MODE1)){
	...
	#ifdef CPUSPIN
	if(cpuspintime != 0){
		if(rt_task_create(&CpuSpin,"Cpu Spin", 0, CPUSPIN_TASK_PRIORITY, ECATCTRL_TASK_MODE2)){
